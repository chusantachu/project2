	...
	<c:if test="${not empty list }">
			<c:forEach var="vo" items="${list }">
				<tr>
					...
					<td>
						<button type="submit" class="btn btn-dark" onclick="location.href='admin?type=admin-orderDetail&oNum=${vo.oNum}&cPage=${pvo.nowPage }'">VIEW</button>
					</td>
				</tr>
			</c:forEach>
		</c:if>
...
<div class="center">
	<div id="page">
		...
		<c:if test="${pvo.beginPage != 1}">	
			<li>
					<a href="admin?type=admin-order&cPage=${pvo.beginPage - 1 }">&nbsp;&lt;&nbsp;</a> 
			</li>
		</c:if>
		...
		<c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
			...
			<c:if test="${pageNo != pvo.nowPage }">
	 			<li class="notnow"><a href="admin?type=admin-order&cPage=${pageNo }">${pageNo}</a></li> 
			</c:if>
		</c:forEach>	
		
		<c:if test="${pvo.endPage < pvo.totalPage}">
			<li>
					<a href="admin?type=admin-order&cPage=${pvo.endPage + 1 }">&nbsp;&gt;&nbsp;</a>
			</li>
		</c:if>
		<c:if test="${pvo.endPage >= pvo.totalPage}">
			<li class="disable">&nbsp;&gt;&nbsp;</li>
