 		...
 		<c:if test="${not empty nowpageList}">
			<c:forEach var="vo" items="${nowpageList }"> 
				<tr>
					...
					<td>${vo.id }</td>
					<td>
						<a class="link" href="admin?type=admin-qnaDetailSearch&qNum=${vo.qNum}&cPage=${cPage}&select=${select}&keyword=${keyword}" style="font-size: 1em; color: black;">
							${vo.qTitle }
						</a>
					</td>
...
<div class="center">
	<div id="page">
		...
		<c:if test="${pvo.beginPage != 1}">	
			<li>
					<a href="admin?type=admin-qnaList&cPage=${pvo.beginPage - 1 }&select=${select}&keyword=${keyword}">&nbsp;&lt;&nbsp;</a> 
			</li>
		</c:if>
		...
		<c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
			...
			<c:if test="${pageNo != pvo.nowPage }">
	 			<li class="notnow"><a href="admin?type=admin-qnaList&cPage=${pageNo }&select=${select}&keyword=${keyword}">${pageNo}</a></li> 
			</c:if>
		</c:forEach>	
		
		<c:if test="${pvo.endPage < pvo.totalPage}">
			<li>
					<a href="admin?type=admin-qnaList&cPage=${pvo.endPage + 1 }&select=${select}&keyword=${keyword}">&nbsp;&gt;&nbsp;</a>
			</li>
		</c:if>
		<c:if test="${pvo.endPage >= pvo.totalPage}">
			<li class="disable">&nbsp;&gt;&nbsp;</li>
