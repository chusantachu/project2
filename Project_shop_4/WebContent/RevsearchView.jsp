...
<%@page import="com.mystudy.paging.Paging" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
			...
			<c:otherwise>
				<c:forEach var="vo" items="${list }">
					<tr>
						<td>${vo.bNum }</td>
						<td>
						<a href="Rview.jsp?bNum=${vo.bNum }&cPage=${pvo.nowPage}&id=${id}">${vo.title }</a> </td>
						<td>${vo.id }</td>
						<td>${vo.bDate }</td>
						<td>${vo.hits }</td>
					</tr>
			...
			<tr>
				<td colspan="4"  style="border-bottom: none;">
					<ol class="paging">
						...
						<c:if test="${pvo.beginPage != 1 }">
								<li><a href="bord?type=revSearch&cPage=${pvo.beginPage -1 }&keyword=${keyword }&idx=${idx}&id=${id}">이전으로</a></li>
						</c:if>
						...
						<c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
								...
								<c:if test="${pageNo != pvo.nowPage }">
									<li><a href="bord?type=revSearch&cPage=${pageNo }&keyword=${keyword }&idx=${idx}&id=${id}">${pageNo }</a></li>
								</c:if>
						</c:forEach>
						<c:if test="${pvo.endPage < pvo.totalPage }">
								<li><a href="bord?type=revSearch&cPage=${pvo.endPage +1}&keyword=${keyword }&idx=${idx}&id=${id}">다음으로</a></li>
							</c:if>
				...
				<td style="border-bottom: none;">
					<input type="button" value="글쓰기" class="btn btn-dark" onclick='javascript:location.href="revWrite.jsp?id=${id}"'>
				</td>
			</tr>
			<tr>
