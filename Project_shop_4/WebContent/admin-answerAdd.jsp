<%@page import="com.mystudy.Project_shop.dao.AdminQnaAnswerDAO"%>
<%@page import="com.mystudy.Project_shop.vo.AdminQnaAnswerVO"%>
...
<script>
	alert("기존 답변내역이 존재하여 해당 게시글 답변으로 이동합니다.");
 	location.href = "admin?type=admin-answerDetailAdd&qaNum=" + <%=vo3.getQaNum()%> + "&qNum=" + <%=vo3.getqNum()%>;
</script>
<%		
	} 
%>
...
<%
	...
	vo2 = AdminAnswerDAO.selectAnswerQnum(qNum);
	if (!id.equals("admin")) {
		if (vo2 == null) {
%>
		<script>
		alert("해당 게시글 답변이 존재하지 않아 이전페이지로 이동합니다.");
		history.back();
		</script>
<%			
	} else {
